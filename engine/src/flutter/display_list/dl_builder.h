...
class DisplayListBuilder final : public virtual DlCanvas,
 ...
 public:
  ...
  void ClipRect(const DlRect& rect,
                DlCanvas::ClipOp clip_op = ClipOp::kIntersect,
  ...
  void DrawImageRect(
      ...
      DlImageSampling sampling,
      const DlPaint* paint = nullptr,
      DlCanvas::SrcRectConstraint constraint = SrcRectConstraint::kFast) override;
  // |DlCanvas|
 ...
 private:
  ...
  // |DlOpReceiver|
  void clipRect(const DlRect& rect, DlCanvas::ClipOp clip_op, bool is_aa) override {
    ClipRect(rect, clip_op, is_aa);
  }
  // |DlOpReceiver|
  void clipOval(const DlRect& bounds, DlCanvas::ClipOp clip_op, bool is_aa) override {
  ...
  // |DlOpReceiver|
  void clipPath(const DlPath& path, DlCanvas::ClipOp clip_op, bool is_aa) override {
  ...
  // |DlOpReceiver|
  void drawPoints(DlCanvas::PointMode mode, uint32_t count, const DlPoint pts[]) override;
  // |DlOpReceiver|
  void drawVertices(const std::shared_ptr<DlVertices>& vertices,
  ...
  void drawImageRect(
      ...
      DlCanvas::SrcRectConstraint constraint = DlCanvas::SrcRectConstraint::kFast) override;
  // |DlOpReceiver|
  void drawImageNine(const sk_sp<DlImage> image,
                     const DlIRect& center,
                     const DlRect& dst,
  ...
  static DisplayListAttributeFlags FlagsForPointMode(DlCanvas::PointMode mode);
